...
import { useGetAllZonesQuery, useCreateZoneMutation, useUpdateZoneMutation } from "@/redux/api/zoneApi";
...
import ZoneDrawingMap from "@/components/modules/dashboard/ZoneDrawingMap";
  ...
  return (
    <div className="bg-zinc-900/50 p-6 md:p-8 rounded w-full border border-zinc-800 shadow-xl backdrop-blur-sm">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="bg-zinc-900/80 p-4 rounded border border-zinc-800 text-white shadow-inner">
          <AddressInput
            register={register}
            setValue={setValue}
        ...
        <div className="flex items-center justify-end gap-3 pt-8 mt-4 border-t border-zinc-800">
          <button 
            type="button" 
            onClick={() => router.push("/dashboard/zones")} 
            className="px-6 py-2.5 text-sm font-medium text-zinc-300 bg-zinc-800 hover:bg-zinc-700 rounded transition-colors border border-zinc-700"
          >
            Cancel
          </button>
          <button 
            ...
            className="bg-emerald-600 hover:bg-emerald-500 text-white px-8 py-2.5 rounded transition-all shadow-[0_0_15px_rgba(16,185,129,0.3)] hover:shadow-[0_0_25px_rgba(16,185,129,0.5)] font-bold text-sm"
          >

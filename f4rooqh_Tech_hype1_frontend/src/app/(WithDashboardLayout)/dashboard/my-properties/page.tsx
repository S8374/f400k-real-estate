...
export default function MyPropertiesPage() {
  ...
  const isVerified = user?.agentProfile?.isRegaVerified && user?.agentProfile?.isNafathVerified;
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-4">
            <Badge
              className={`px-4 py-1 flex items-center gap-1 ${user?.agentProfile?.isRegaVerified && user?.agentProfile?.isNafathVerified
                ? "bg-emerald-700 hover:bg-emerald-600"
                : "bg-red-600 hover:bg-red-500"
                }`}
            >
              <Shield className="w-4 h-4" />
              {user?.agentProfile?.isRegaVerified && user?.agentProfile?.isNafathVerified
                ? "Verified"
                : "Not Verified"}
            </Badge>
          </div>

...
import { generateToken, verifyToken } from '../../helper/jwt/jwtHelper';
...
export class AuthService {
  ...
  async login(dto: LogInAuthDto, res: Response) {
    ...
    if (!user.password) {
      throw new UnauthorizedException(
        'Account has no password set. Please use social login or reset your password.',
      );
    }
  ...
  async adminLogin(dto: LogInAuthDto, res: Response) {
    const { email, password } = dto;
    ...
    const tokenInfo: AuthTokens = { accessToken, refreshToken };
    setAdminAuthCookie(res, tokenInfo);

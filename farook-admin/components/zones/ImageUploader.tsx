...
interface ImageUploaderProps {
  ...
  maxFiles?: number;
  accept?: string;
  videoProxyUrl?: (url: string) => string;
}
  ...
  return (
    <div className="space-y-2">
      ...
      >
        ...
        <p className="text-xs text-gray-500 mt-1">Images (PNG, JPG) and Videos (MP4) up to 10MB</p>
      </div>
      ...
      {previews.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-4">
          ...
          {existingUrls.map((url, idx) => {
            const videoSrc = videoProxyUrl ? videoProxyUrl(url) : url;
            return (
              <div key={`url-${idx}`} className="relative group">
                {isVideoUrl(url) ? (
                  <video
                    src={videoSrc}
                    className="rounded object-cover w-full h-32 bg-black"
                    controls
                    muted
                  />
              ) : (
